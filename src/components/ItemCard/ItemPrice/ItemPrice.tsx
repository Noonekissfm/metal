    ...
    return (
        <div className={`ItemPrice__wrapper--${primary && !!price? 'primary' : 'secondary'}`}>
            ...
            <>
                ...
                <div className='ItemPrice__order'><p>Сделать заказ можно по телефону:{'\u00A0'}</p><Contact icon={false} contact={company.phone} type={'phone'} /></div>
            </>:
            <><p>Уточнить стоимость можно по телефону:{'\u00A0'}</p><Contact icon={false} contact={company.phone} type={'phone'} /></>}
        </div>
    );
};
